...
const CompanyCard: React.FC<CompanyCardProps> = ({
  ...
  return (
    <div
      className={`flex flex-col md:flex-row ${isReverse ? 'md:flex-row-reverse' : ''} items-center gap-8 md:gap-16 py-8 md:py-12 w-full justify-center`}
    >
      {/* Logo and Text */}
      <div className="flex-1 flex flex-col gap-4 items-center md:items-start text-center md:text-left max-w-full md:max-w-md w-full px-2 md:px-0">
        <div className="flex items-center gap-3 justify-center md:justify-start w-full">
          <div className="bg-white rounded-lg p-2 shadow w-[120px] h-[48px] md:w-[200px] md:h-[56px] flex items-center justify-center mx-auto md:mx-0">
            <Image
              src={logo}
              alt={logoAlt}
              width={200}
          ...
          <span className="font-bold text-base md:text-lg text-white truncate w-32 md:w-auto">
            {title}
          </span>
        </div>
        <div className="text-[#F6E7B7] font-bold text-base md:text-xl">{subtitle}</div>
      ...
      {/* Image */}
      <div className="flex-1 flex justify-center items-center w-full max-w-full md:max-w-xl px-2 md:px-0 mt-6 md:mt-0">
        <div className="rounded-2xl shadow-lg p-1 md:p-2 w-full">
          <Image
            src={image}
            alt={imageAlt}
            width={600}
            height={340}
            className="rounded-xl object-cover w-full h-auto max-h-56 md:max-h-[340px]"
          />
        </div>
      </div>
    </div>
